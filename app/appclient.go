...
func (appClient *APPClient) InitAPPClient() error {
	...
	appClient.EventHub.RegisterBlockEvent(BlockCallBack)
	return nil
}

func BlockCallBack(block *common.Block) {
	// parse block
	...
	case common.HeaderType_ENDORSER_TRANSACTION:
		...
		go CollectBlockLog(bLogsChan, block.Header.Number, int64(len(tx.Actions)))
		break
...
func (context *CurrentContext) Invoke(args []string) (string, error) {
	...
	select {
	case <-done:
		go CollectTxLog(txLogsChan, args[0], "success", (time.Now().UnixNano() - timeStart.UnixNano()))
	case <-fail:
		go CollectTxLog(txLogsChan, args[0], "fail", (time.Now().UnixNano() - timeStart.UnixNano()))

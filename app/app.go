...
func buildInsuranceRouter() *web.Router {
	...
	for currentChannelK, _ := range channels {
		...
		for _, currentPeerOrg := range currentChannelPeers {
			...
			app.Post("/insertTranData", (*InsuranceAPP).postTLog)
			app.Post("/insertBlockData", (*InsuranceAPP).postBLog)
		}
	}
...
func (s *InsuranceAPP) postTLog(rw web.ResponseWriter, req *web.Request) {
	logger.Infof("postTLog TLog start")
	encoder := json.NewEncoder(rw)
	var result LogResponseData
	// Decode the incoming JSON payload
	body, err := ioutil.ReadAll(req.Body)
	if err != nil {
		result.StatusCode = 1
...
func (s *InsuranceAPP) postBLog(rw web.ResponseWriter, req *web.Request) {
	var result LogResponseData
	encoder := json.NewEncoder(rw)
	// Decode the incoming JSON payload
	body, err := ioutil.ReadAll(req.Body)
...
guolidong:~/go/src/github.com/hyperledger/delayinsurance/fixtures/config$ cat client.crt client.key> client.pem
guolidong:~/go/src/github.com/hyperledger/delayinsurance/fixtures/config$ cat server.crt server.key > server.pem
guolidong:~/go/src/github.com/hyperledger/delayinsurance/fixtures/config$ openssl pkcs12 -export -inkey client.key -in client.crt -out client.pfx
Enter Export Password:
Verifying - Enter Export Password:
unable to write 'random state'
guolidong:~/go/src/github.com/hyperledger/delayinsurance/fixtures/config$ openssl pkcs12 -export -inkey server.key -in server.crt -out server.pfx
Enter Export Password:
Verifying - Enter Export Password:
unable to write 'random state'
guolidong:~/go/src/github.com/hyperledger/delayinsurance/fixtures/config$ ls
 */
